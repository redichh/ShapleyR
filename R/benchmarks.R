...
#'
test.convergence = function(row.nr=2, convergence.iterations = 20, iterations = 20, task = agri.task,
                            model = train(makeLearner("cluster.kmeans", predict.type = "prob"), agri.task),
                            return.value = "values") {

  assert_number(convergence.iterations)
  assert_numeric(row.nr)
