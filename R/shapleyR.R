...
shapley = function(row.nr, learner = makeLearner("regr.lm"), task = bh.task,
  ...
  if(method == "regr") {
    ...
    result = shapley.clustering(row.nr, learner, task, model, iterations)
  } else if (method == "unsampled") {
    print("Work in progress")
    #FIXME: implement unsampled version
...
shapley.regr = function(row.nr, task = bh.task, model = train("regr.lm", bh.task), iterations = 50) {
...
shapley.classif = function(row.nr, learner = makeLearner("classif.lda", predict.type = "prob"),
  ...
  for(feature in getTaskFeatureNames(task)) {
    ...
    predict_b1 = getPredictionProbabilities(predict(model, newdata=b1), getTaskClassLevels(task))
    predict_b2 = getPredictionProbabilities(predict(model, newdata=b2), getTaskClassLevels(task))
...
shapley.clustering = function(row.nr, learner = makeLearner("cluster.kmeans", predict.type = "prob"),
  task = mtcars.task, model = train(learner, task), iterations = 50) {

  assert_set_equal(getTaskType(task), "cluster")
  assert_set_equal(getLearnerPredictType(learner), "prob")
